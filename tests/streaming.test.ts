...
describe("Streaming responses", () => {
  it("returns a live Response before the source body completes", async () => {
    ...
    const response = await api.get("/events").asStream();
  ...
  it("preserves Fetch response metadata and clone behavior", async () => {
    const api = lafetch.create();
    const response = await api.get("data:text/plain,hello").asStream();
  ...
  it("owns one consumption mode per Builder", async () => {
    ...
    const streaming = request.asStream();

    await expect(request.asStream()).rejects.toBeInstanceOf(HttpConsumptionError);
    await expect(request.asRaw()).rejects.toBeInstanceOf(HttpConsumptionError);
    await (await streaming).body?.cancel();

    const buffered = api.get("/resource");
    await buffered.asRaw();
    await expect(buffered.asStream()).rejects.toBeInstanceOf(HttpConsumptionError);
  });
  ...
  it("rejects Schema, Cache, and Deduplication before Transport execution", async () => {
    ...
    await expect((api.get("/schema").validate((value) => value) as any).asStream())
      .rejects.toBeInstanceOf(HttpConfigurationError);
    await expect((api.get("/cache").cache("1m") as any).asStream())
      .rejects.toBeInstanceOf(HttpFeatureConflictError);
    await expect((api.get("/dedupe").dedupe() as any).asStream())
      .rejects.toBeInstanceOf(HttpFeatureConflictError);
    await expect(api.get("/custom-cache").use({
      name: "custom-cache",
      capabilities: { provides: [{ name: "cache" }] },
    }).asStream()).rejects.toBeInstanceOf(HttpFeatureConflictError);
  ...
  it("rejects a response body already consumed by a Feature", async () => {
    ...
    }).asStream()).rejects.toBeInstanceOf(HttpConsumptionError);
  ...
  it("keeps Streaming unbounded unless maxResponseBytes() is explicit", async () => {
    ...
    await expect(api.get("/unbounded").asStream().then((response) => response.arrayBuffer()))
