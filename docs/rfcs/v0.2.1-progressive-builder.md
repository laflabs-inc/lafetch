...
`asJson()`, `asText()`, `asArrayBuffer()`, `asBlob()`, `asFormData()`, `asResponse()`, `asRaw()`는 실제 `Promise`를 반환합니다. 응답 데이터 타입은 `get<T>()`, `post<T>()`, `request<T>()` 같은 HTTP 진입 메서드에서 한 번만 선언하며 `asJson<T>()` 형태는 허용하지 않습니다. 기존 `as(type)`, `response()`, `raw()`는 알파 공개 API에서 제거합니다.
...
명시적인 `as*()` 메서드는 Builder 상태를 바꾸지 않고 실제 Promise를 반환하는 terminal입니다. v0.2.1에는 Streaming terminal을 추가하지 않습니다. 이 단방향 상태 전이는 v0.3에서 buffered와 streaming의 잘못된 체인 조합을 IDE에서 제거하기 위한 타입 기반을 제공하지만, 미래 공개 메서드 이름까지 고정하지는 않습니다.
