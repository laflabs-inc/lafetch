# Lafetch v0.1에서 v0.2로 마이그레이션

v0.2는 호환 별칭을 추가하는 버전이 아니라 요청 문법을 하나로 통일하는 공개 API 재설계입니다. 기존 코드와 새 코드를 섞기보다 아래 순서대로 한 번에 전환하는 것을 권장합니다.
...
5. 사용자 Feature 타입과 Helper import를 `@laflabs/lafetch/feature`로 이동합니다.
...
상태 코드와 헤더가 필요할 때만 `response()`를 사용합니다.
...
| `request.send()` | `request.response()` | 전체 응답이 필요할 때만 사용 |
| `request.jsonBody(value)` | `request.json(value)` | JSON 요청 본문 |
| `request.json<T>()` | `await request` 또는 `.as("json")` | 응답 형식은 실행 전에 선언 |
| `request.text()` | `request.as("text")` | 다른 형식도 `.as()`로 통일 |
...
v0.2의 `json(value)`는 요청 본문만 설정합니다. JSON 응답은 기본 자동 디코딩에 맡기고, 서버의 `Content-Type`을 신뢰할 수 없을 때만 `.as("json")`을 사용합니다.
...
| 설정 | 허용 값 |
| --- | --- |
| `as()` | `auto`, `json`, `text`, `arrayBuffer`, `blob`, `formData` |
| Credentials | `omit`, `same-origin`, `include` |
| Backoff `type` | `fixed`, `exponential` |
| Backoff `jitter` | `none`, `full` |

문자열 축약형 `backoff: "fixed"`나 알 수 없는 값은 기본 동작으로 대체되지 않고 Transport 실행 전에 `HttpConfigurationError`를 발생시킵니다.
...
- 응답 종결 메서드를 `await`, `as()`, `response()`, `raw()` 중 의도에 맞게 변경합니다.
